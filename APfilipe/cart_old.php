...
<div id="content">
	...
	<div>
		<table style="width:calc(100% - 15px);border: #ccccca solid 1px"> 
			...
			<?php $price = 0; $qty = 0; $totalItems = 0;
			for ($i=0; $i<99; $i++) {
				$name1 ='cartItem'; 
				$name2 = $name1.$i;
						...
						while ($row = $result->fetch_assoc ()) { 
							...
							if ($item == $sid){ 
								...
								<tr class="hl">
									...
									<td style="text-align: center"> <?PHP echo $row['lpa_stock_name'];?></td>
									<td style="text-align: center"> <?php $qty = 1;?><?php echo $qty ?> </td>
									<td style="text-align: center" width= "35" height="50">
										<div class="productListItemImageFrame" style="align: center; background:url('images/<?PHP echo $prodImage; ?> ') no-repeat center center;"></div>
									</td>
									<td style="text-align: center"> <?PHP echo $row['lpa_stock_price'];?> </td>
									<td style="text-align: center"> 
										<button id="btndel" name="btndel" onclick="deleteCookie('<?PHP echo $name2; ?>'); navMan('Cart.php') "> Remove </button> 
									</td>
								</tr> 
								<?php $price = ($price + $row['lpa_stock_price']); 
							}
						} 
					} 
				}
				...
				<tr class="PageTitle" style="background: #eeeeee" >
					...
					<td style="width: 80px; text-align: right"></td>
					<td style="width: 80px; text-align: center"><?php echo $price; ?> </td>
					<td style="width: 80px; text-align:right"></td>
				</tr>
				<tr><td></td></tr><tr><td></td></tr><tr><td></td></tr><tr><td></td></tr><tr><td></td></tr>
				<tr> 
					<td>
						<button style="width: 120px; height:40px" type="button" id="btnback" onclick="navMan('products.php')">Back to Catalog </button>
					</td>
					<td></td><td></td><td></td><td></td>
					<?php if ($totalItems > 0) { ?>
						<td>
							<button style="width: 120px; height: 40px" type="button" id="btnAddRec" onclick="setCookie('totalItems',<?php echo $totalItems; ?>,1); setCookie('price',<?php echo $price; ?>,1); navMan('Payment.php') ">Confirm</button> 
						</td> 
					<?php } else { ?> 
					<td></td> <?php } ?>
				</tr>

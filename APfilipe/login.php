<?PHP 
  ...
  if($action == "doLogin") {
    ...
    openDB();
    $query =
      "SELECT lpa_user_ID, lpa_user_username, lpa_user_password,lpa_user_group
      FROM lpa_users
      WHERE lpa_user_username = '$uName'
        AND lpa_user_password = '$hashed_password'
		AND lpa_user_status   <> 'D'
      LIMIT 1";
    ...
    <form name="frmLogin" id="frmLogin" method="post" action="login.php">
      ...
      <div id="loginFrame">
        ...
        <div class="buttonBar">
		  ...
		  <button type="button" id="btnRegRec" onclick="">New User</button>
        </div>
      </div>
...
<script>
  ...
  $("#frmLogin").keypress(function(e) {
	...
	function loadRegItem (ID,MODE) {
		window.location = "reg.php?sid=" + ID + "&a=" + MODE;
	} 
